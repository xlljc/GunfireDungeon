...
{
    // 不同类型对护盾/装甲/生命的倍率（摘自文档表格）
    private static readonly float[,] DamageMatrix =
    {
        // Shield, Armor, Health
        { 1.0f, 1.0f, 1.0f }, // Physical
        { 1.0f, 0.5f, 1.5f }, // Fire
        { 1.5f, 1.0f, 0.5f }, // Electric
        { 0.5f, 1.5f, 1.0f }, // Chemical
        { 1.0f, 1.5f, 0.5f }, // Optical
        { 1.5f, 0.5f, 1.0f }, // DarkMatter
        { 0.5f, 1.0f, 1.5f }  // Explosive
    };
    
    // 默认减免上限
    private static readonly float DefaultMaxReduction = 0.8f;
    // 默认减免下限
    private static readonly float DefaultMinReduction = -1;

    private static Random _rand = new Random();

    public static void ApplyDamage(CharacterStats target, AttackStats attack)
    {
        if (target.IsDead)
        {
            GD.Print("目标已死亡。");
